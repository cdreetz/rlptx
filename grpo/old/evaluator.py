...
def get_evaluator(name: str) -> RewardEvaluator:
    ...
    """
    if name.lower() == "nlkernel":
        return NLKernelEvaluator()
    else:
        raise NotImplementedError(f"No evaluator implemented for {name}")



class NLKernelEvaluator(RewardEvaluator):
    """
    Reward evaluator for the kernel book + NL adaption dataset.
    
    Implements reward functions for:
    - Compiles
    - Correctness vs CPU version
    - Applies expected optimizations
    ...
    def _compiled_reward(self, prompts, completions, answer) -> List[float]:
        """Reward for compiled kernel."""
        responses = [completion[0]['content'] for completion in completions]
        extracted = [self._extract_xml_answer(r) for r in responses]
        return [2.0 if r == a else 0.0 for r, a in zip(extracted, answer)]

    def _verified_correct(self, completions) -> List[float]:
        """Reward for correctness against CPU implementation."""
        responses = [completion[0]['content'] for completion in completions]
        extracted = [self._extract_xml_answer(r) for r in responses]
