...
class SFTTrainingSetup:
    ...
    def train_epoch(self, model, dataloader, optimizer, epoch, gradient_accumulation_steps=1):
        """Train for one epoch"""
        model.train()
        total_loss = 0
        accumulated_loss = 0
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            loss.backward()
            accumulated_loss += original_loss
        ...
        num_grad_steps = len(dataloader) // gradient_accumulation_steps
        epoch_avg_loss = total_loss / num_grad_steps
