...
class TritonSandbox:
    ...
    def _generate_dynamic_run_function(self, launch_funcs: List[str]) -> List[str]:
        ...
        run_function = [
            ...
            "    ",
            "    # Get the docstring to extract tensor dimension information",
            f"    docstring = {launch_func_name}.__doc__ or ''",
            "    print('Function docstring:', repr(docstring))",
            "    ",
            "    # Parse docstring for tensor dimension information",
            "    def parse_tensor_dims_from_docstring(docstring):",
            "        '''",
            "        Parse tensor dimension information from docstring.",
            "        ",
            ...
            "        tensor_dims = {}",
            "        ",
            "        # Pattern 1: '- param: ... tensor of shape (dim1, dim2, ...)'",
            "        pattern1 = r'-\s*(\w+):\s*.*?tensor.*?of\s+shape\s*\(([^)]+)\)'",
            "        ",
            "        # Pattern 2: 'param (torch.Tensor): shape (dim1, dim2, ...)'",
            "        pattern2 = r'(\w+)\s*\([^)]*torch\.Tensor[^)]*\):\s*.*?shape\s*\(([^)]+)\)'",
            "        ",
            "        # Pattern 3: 'param: torch.Tensor of shape (dim1, dim2, ...)'",
            "        pattern3 = r'(\w+):\s*torch\.Tensor.*?of\s+shape\s*\(([^)]+)\)'",
            "        ",
            "        # Pattern 4: '- param: ... shape (dim1, dim2, ...)' (more general)",
            "        pattern4 = r'-\s*(\w+):\s*.*?shape\s*\(([^)]+)\)'",
            "        ",
            "        patterns = [pattern1, pattern2, pattern3, pattern4]",
            "        ",
            "        for pattern in patterns:",
            "            matches = re.findall(pattern, docstring, re.IGNORECASE | re.MULTILINE)",
            "            for param_name, dims_str in matches:",
            "                # Parse dimensions",
            "                dims = [d.strip() for d in dims_str.split(',')]",
            "                # Convert dimension names to actual sizes",
            "                dim_sizes = []",
            "                for dim in dims:",
            "                    dim = dim.strip()",
            "                    if dim.lower() in ['n', 'batch_size', 'batch']:",
            "                        dim_sizes.append(2)",
            "                    elif dim.lower() in ['c', 'channels', 'channel']:",
            "                        dim_sizes.append(3)",
            "                    elif dim.lower() in ['h', 'height']:",
            "                        dim_sizes.append(32)",
            "                    elif dim.lower() in ['w', 'width']:",
            "                        dim_sizes.append(32)",
            "                    elif dim.lower() in ['d', 'depth']:",
            "                        dim_sizes.append(16)",
            "                    elif dim.lower() in ['seq_len', 'sequence_length', 'length']:",
            "                        dim_sizes.append(128)",
            "                    elif dim.lower() in ['vocab_size', 'vocab']:",
            "                        dim_sizes.append(1000)",
            "                    elif dim.lower() in ['hidden_size', 'hidden']:",
            "                        dim_sizes.append(256)",
            "                    else:",
            "                        # Default size for unknown dimensions",
            "                        dim_sizes.append(8)",
            "                ",
            "                if dim_sizes:",
            "                    tensor_dims[param_name] = tuple(dim_sizes)",
            "                    print(f'Parsed {param_name}: shape {tuple(dim_sizes)} from docstring')",
            "        ",
            "        return tensor_dims",
            "    ",
            ...
            "    ",
            "    # Parse tensor shapes from docstring",
            "    tensor_shapes = parse_tensor_dims_from_docstring(docstring)",
            "    ",
            "    # Generate example arguments based on type annotations and docstring info",
            "    args = {}",
            ...
            "        ",
            "        if (annotation == torch.Tensor or str(annotation) == \"<class 'torch.Tensor'>\" or",
            "            'Tensor' in str(annotation) or annotation == inspect.Parameter.empty):",
            "            ",
            "            # Use shape from docstring if available",
            "            if param_name in tensor_shapes:",
            "                shape = tensor_shapes[param_name]",
            "                tensor = torch.rand(shape, device=DEVICE, dtype=torch.float32)",
            "                print(f'Generated {param_name}: torch.Tensor of shape {tensor.shape} (from docstring)')",
            "            else:",
            "                # Default to 1D tensor",
            "                tensor = torch.rand(default_size, device=DEVICE, dtype=torch.float32)",
            "                print(f'Generated {param_name}: torch.Tensor of shape {tensor.shape} (default 1D)')",
            "            args[param_name] = tensor",
            "        ",
            "        elif annotation == int or annotation == 'int':",
            "            # Smart defaults for common parameter names",
            "            if 'kernel' in param_name.lower() and 'size' in param_name.lower():",
            "                value = 3",
            "            elif 'stride' in param_name.lower():",
            "                value = 1",
            "            elif 'pad' in param_name.lower():",
            "                value = 0",
            ...
            "            if len(tensor_args) >= 2 and all(t.shape == tensor_args[0].shape for t in tensor_args[1:]):",
            ...
            "            else:",
            "                print('  Skipping operation validation (different tensor shapes or single tensor)')",
            "        else:",
